...
> **The built `.amxd` is not distributed — build your own.** Max bakes the `[js]`
> object's argument into the device as an **absolute** path, so a built device carries
> whoever built it and works on no other machine. Only the `.maxpat` source is tracked,
> and it carries no such path. (The host reads the device's declared command-file path
> back out of the installed `.amxd` — see `host/tap.py`.)
>
> *Note: the build command and the `/tmp/...` examples below are inherited from the
> upstream fork and have not been updated for this project's layout yet.*
