...
def handle(context: dict, request: dict) -> dict:
    """context = {roots, versions:{live,python}, capabilities}."""
    rid = request.get("id")
    ...
    try:
        return {"id": rid, "ok": True, "result": handler(context, params)}
...
# --- methods (the Phase-1 generic core) ---------------------------------------

def _hello(context, params):
